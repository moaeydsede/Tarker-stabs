...
const screenMeds = $("screenMeds");
const navToday = $("navToday");
const navMeds = $("navMeds");
const btnGoMeds = $("btnGoMeds");
const medsList = $("medsList");
const medsEmpty = $("medsEmpty");
const medsSearchInput = $("medsSearchInput");
...
let activeScreen = "today"; // today | meds

function showScreen(name){
  activeScreen = name;
  if (screenToday) screenToday.classList.toggle("hidden", name !== "today");
  if (screenMeds) screenMeds.classList.toggle("hidden", name !== "meds");
  if (navToday) navToday.classList.toggle("is-active", name === "today");
  if (navMeds) navMeds.classList.toggle("is-active", name === "meds");
}
...
function renderMeds(){
  if (!medsList || !medsEmpty) return;

  const q = (medsSearchInput && medsSearchInput.value ? medsSearchInput.value : "").trim();

  const all = state.meds
    .slice()
  ...
  for (const med of all){
    ...
    img.className = "pill";
    img.alt = med.name || "دواء";
    img.src = med.imageDataUrl || "icon-192.png";
    ...
    title.textContent = med.name || "بدون اسم";
    meta.appendChild(title);

    const subtitle = document.createElement("div");
    subtitle.className = "item__sub";
    const t = (med.type === "prn") ? "أزمة/عند اللزوم" : "بوقت ثابت";
    const times = (med.type === "prn") ? "" : ` • ${((med.times||[]).join(" ، ")) || "—"}`;
    const note = med.note ? ` • ${med.note}` : "";
    subtitle.textContent = `${t}${times}${note}`;
    ...
    btnEdit.addEventListener("click", ()=> openEdit(med.id));
...
if (btnGoMeds){
  btnGoMeds.addEventListener("click", ()=> showScreen("meds"));
}
...
medForm.addEventListener("submit", async (e)=>{
  ...
  render();
  showScreen("today");
  if (navToday) navToday.addEventListener("click", ()=> showScreen("today"));
  if (navMeds) navMeds.addEventListener("click", ()=> showScreen("meds"));
  if (medsSearchInput) medsSearchInput.addEventListener("input", renderMeds);
  startReminderLoop();
});
...
async function notifyNow(title, body){
  try{
    ...
    // استخدم SW showNotification دائمًا (بدون new Notification) لتجنب Illegal constructor
